...
scheduler.runTaskAsynchronously(this, new Runnable() {
  ...
  public void adoptPet(Player player, String pet_name) {
	...
	if ((user.countEmeralds()>= new Double(PET_PRICE / DENOMINATION_FACTOR).intValue())&&(BitQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase("emerald"))) { 
        ...
        if (entity instanceof Ocelot) {
          if (entity.getCustomName() != null && entity.getCustomName().equals(cat_name)) {
            if (cat_is_found == false) {
              ...
              cat_is_found = true;
            } if (BitQuest.REDIS.get("petIsOn"+player.getUniqueId().toString()).equalsIgnoreCase("off")) {
	    entity.remove();
	    } else {
              entity.remove();
            }
      ...
      if (!hasNonAlpha) {
        ...
        if (name.length() <= 27) {
          ...
          if (REDIS.get(tempchunk+""+ x + "," + z + "owner") == null) {
            ...
            if ((BITCOIN_NODE_HOST != null)&&(BitQuest.REDIS.get("currency"+player.getUniqueId().toString()).equalsIgnoreCase(BitQuest.DENOMINATION_NAME))) {
              user.wallet.getBalance(
                  0,
                  new Wallet.GetBalanceCallback() {
                    @Override
			 ...
			 { landxprice=4;}
                        if (balance >= (LAND_PRICE*landxprice)) {
                          if (user.wallet.move("land", (LAND_PRICE*landxprice))) {
                            ...
                            if (balance < (BitQuest.LAND_PRICE*landxprice)) {
                              player.sendMessage(
                                  ChatColor.DARK_RED
                                      ...
                                      + (int)
                                          Math.ceil(
                                              ((BitQuest.LAND_PRICE*landxprice) - balance)
                          ...
                          player.sendMessage(
                              ChatColor.DARK_RED
                                  ...
                                  + (int)
                                      Math.ceil(
                                          (BitQuest.LAND_PRICE*landxprice) / BitQuest.DENOMINATION_FACTOR)
                                  + ChatColor.DARK_RED
                                  + BitQuest.DENOMINATION_NAME);
                        }
                      } catch (Exception e) {
