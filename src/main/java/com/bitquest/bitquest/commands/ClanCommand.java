                        ...
                        if (clanName.length() <= 16) {
                            if (!BitQuest.REDIS.exists("clan:" + player.getUniqueId().toString())) {
                                if (!BitQuest.REDIS.sismember("clans", clanName)) {
                                    ...
                                    player.sendMessage(
                                            ChatColor.GREEN + "Congratulations! you are the founder of the " + clanName + " clan");
                                ...
                                player.sendMessage(
                                        ChatColor.RED + "You already belong to the clan " + BitQuest.REDIS.get("clan:" + player.getUniqueId().toString()));
                                return true;
                            }
                        } else {
                        ...
                        if (BitQuest.REDIS.exists("uuid:" + invitedName)) {
                            ...
                            if (!BitQuest.REDIS.exists("clan:" + uuid)) {
                                ...
                                if (!BitQuest.REDIS.sismember("invitations:" + clan, uuid)) {
                                    ...
                                    player.sendMessage(
                                            ChatColor.RED + "Player " + invitedName + " is already invited to the clan and must accept the invitation");
                                    return true;
                                }
                                ...
                                if (BitQuest.REDIS.get("clan:" + uuid).equals(clan)) {
                                    player.sendMessage(
                                            ChatColor.RED + "Player " + invitedName + " already belongs to the clan " + clan);
                    ...
                    if (BitQuest.REDIS.exists("uuid:" + toKick)) {
                        ...
                        if (BitQuest.REDIS.exists("clan:" + player.getUniqueId().toString())) {
                            ...
                            if (BitQuest.REDIS.get("clan:" + uuid).equals(clan)) {
                                ...
                                player.sendMessage(
                                        ChatColor.GREEN + "Player " + toKick + " was kicked from the " + clan + " clan.");
