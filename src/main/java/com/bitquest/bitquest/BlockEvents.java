...
/** Created by explodi on 11/1/15. Edited by Xeyler on 11/2/15 */
public class BlockEvents implements Listener {
  ...
  void onBlockBreak(BlockBreakEvent event) {
    ...
    if (event.getBlock().getType().equals(Material.BEDROCK)
        || event.getBlock().getType().equals(Material.END_BRICKS)
        || event.getBlock().getType().equals(Material.ENDER_STONE)) {
      event.setCancelled(true);
