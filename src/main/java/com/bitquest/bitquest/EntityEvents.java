...
public class EntityEvents implements Listener {
	...
	public EntityEvents(BitQuest plugin) {
		...
		for (String line : bitQuest.getConfig().getStringList("welcomeMessage")) {
			...
			// Replace the link variable with a minecraft-compatible link
	...
	@EventHandler
	public void onExperienceChange(final PlayerExpChangeEvent event) throws ParseException, org.json.simple.parser.ParseException, IOException {    
		event.setAmount(0);
	}

	@EventHandler
	public void onEnchantItemEvent(final EnchantItemEvent event) throws ParseException, org.json.simple.parser.ParseException, IOException {
	...
	@EventHandler
	public void onPlayerJoin(final PlayerJoinEvent event) throws IOException, org.json.simple.parser.ParseException, ParseException, JSONException {
		...
		if(bitQuest.messageBuilder != null) {
			...
			scheduler.runTaskAsynchronously(bitQuest, new Runnable() {
				@Override
				public void run() {
					final org.json.JSONObject sentEvent = bitQuest.messageBuilder.event(player.getUniqueId().toString(), "Login", null);
					...
					final org.json.JSONObject update = bitQuest.messageBuilder.set(player.getUniqueId().toString(), props);
	...
	@EventHandler
	public void onPlayerLogin(final PlayerLoginEvent event) throws ParseException, org.json.simple.parser.ParseException, IOException {
		...
		if(bitQuest.REDIS.exists("address" + player.getUniqueId().toString()) == false && bitQuest.REDIS.exists("private"+player.getUniqueId().toString()) == false) {
			...
			final URL url = new URL("https://api.blockcypher.com/v1/btc/main/addrs");
	...
	public void onPlayerDeath(final PlayerDeathEvent event) {
		...
		final String spawnkey = spawnKey(event.getEntity().getLocation());
	...
	private String spawnKey(final Location location) {
	...
	@EventHandler
	void onEntityDamage(final EntityDamageEvent event) throws ParseException, org.json.simple.parser.ParseException, IOException {
	...
	public void useRandomEquipment(final LivingEntity entity, final int level) {
		...
		if (BitQuest.rand(0, 32) < level) {
			...
			if (BitQuest.rand(0, 128) < level) 
				helmet = new ItemStack(Material.CHAINMAIL_CHESTPLATE);
			if (BitQuest.rand(0, 128) < level) 
	...
	public static void randomEnchantItem(ItemStack item) {
		...
		if (enchantment != null) {
			final int level = BitQuest.rand(enchantment.getStartLevel(), enchantment.getMaxLevel());
